...
def main():
                ...
                if line['kn'] and model_args.add_kn:
                    if type == 'train':
                        for kn in line['kn']:
                            ...
                            if label == 1:
                                _weights.append(1.0/len(line['kn']))
                            elif label == 0:
                                _weights.append(model_args.negative_weight/len(line['kn']))
                            else:
    ...
    raw_train = read_triple_data(os.path.join(data_args.data_dir, 'train.json'), 'train')
    raw_dev = read_triple_data(os.path.join(data_args.data_dir, 'valid.json'), 'eval')
    raw_test = read_triple_data(os.path.join(data_args.data_dir, 'test.json'), 'eval')
