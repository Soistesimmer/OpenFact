...
class DataTrainingArguments:
    ...
    data_file: Optional[str] = field(
        default=None, metadata={"help": "Need to do predictions."}
    )
    output_file: Optional[str] = field(
        default=None, metadata={"help": "Save predictions."}
    )
...
def main():
    ...
    raw_test = read_triple_data(os.path.join(data_args.data_dir, data_args.data_file))
    ...
    def codex_data_collator(features: List[InputDataClass]) -> Dict[str, Any]:
        ...
        batch = tokenizer.pad({'input_ids': sources, 'token_type_ids': segments},
    ...
    if training_args.do_predict:
        ...
        for predict_dataset in predict_datasets:
            ...
            predictions = F.softmax(torch.tensor(trainer.predict(predict_dataset, metric_key_prefix="predict").predictions), dim=-1)[:, 1]

            output_predict_file = os.path.join(training_args.output_dir, data_args.output_file)
            if trainer.is_world_process_zero():
                with open(output_predict_file, "w") as writer:
                    logger.info(f"***** Predict results *****")
                    # writer.write("index\tprediction\n")
                    for index, score in enumerate(predictions):
                        item = predict_dataset[index]
                        # item = [0, 1][item]
                        writer.write(f"{index}\t{item['head']}\t{item['relation']}\t{item['tail']}\t{score}\n")
